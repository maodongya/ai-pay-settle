...
| 文档 | 说明 | 适用读者 |
|------|------|----------|
| [docs/00-规格说明书.md](docs/00-规格说明书.md) | 功能/非功能/模块/数据/MQ/验收规格（与代码对齐） | 产品、测试、评审 |
...
| [docs/redis使用.md](docs/redis使用.md) | Redis 缓存点、注解规范、序列/锁方案 | 后端开发、架构 |
| [docs/mq/整体优化方案批量写入.md](docs/mq/整体优化方案批量写入.md) | MQ 消费吞吐：真批量写库 + 连接池对齐 | 后端开发、压测 |

## 阅读路径

```
评审/产品  → 00 规格说明书 + 01 架构设计 + 09 异常容错（概要）
后端开工   → 00 规格说明书 → 03 领域字典 → 02 开发手册 → 分库分表 → 按模块读 04~08
上线前必审 → 09 异常与容错设计（全文）
