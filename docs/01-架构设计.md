...
**当前实现（2026-08）：** `AlertService` 写入 `alert_record` + 应用日志。  
**目标态（未落地）：** 操作台站内消息 → 企业微信群 → 邮件（P0 电话）/ Webhook。
...
双人审批场景（目标态）：计费规则变更、结算方案调整、单笔 > 5 万手动出款。  
**当前实现：** 规则 `submit` 落 `status=0` pending；**无审批服务/双人流。**
---
