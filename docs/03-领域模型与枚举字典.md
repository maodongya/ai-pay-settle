...
> 版本：V1.2 | 日期：2026-07-05 | 所有服务共享的领域语言（Ubiquitous Language）
...
CREATE TABLE `merchant_contract` (
  ...
  `sign_date`     date         NOT NULL COMMENT '首次签约日，用于阶梯月数',
  `status`        tinyint      NOT NULL DEFAULT 1 COMMENT '1有效 0注销',
  `settle_mode`   tinyint      NOT NULL DEFAULT 1,
  `min_withdraw`  decimal(18,2) NOT NULL DEFAULT 100.00 COMMENT '最低提现额',
...
库归属：`settlement_db`（contract）、`public_control_db`（profile）。
