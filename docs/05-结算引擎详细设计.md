...
- **并发：** `RedisKeys.settleLock(merchantId)` 分布式锁，事务结束释放。  
...
- `handlePaymentCallback` 使用 `withdrawApplyRepository.updateStatusBySettleNoAndMerchantId`，禁止全表 `findAll`。  
...
- 默认 30 TPS（`pay.db-rate-limit.settlement-tps`）  
