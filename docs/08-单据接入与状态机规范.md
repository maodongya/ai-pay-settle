...
**当前实现（`MerchantValidateServiceImpl`）：**

- `tradeAmount > 0`
- 商户存在且 `status==1`
- 退款：`originBillNo` 非空且原单存在

**目标态（下列伪码，尚未全量落地）：** 金额精度、冻结商户、PROFIT_SHARE 拆分和校验等。

```java
public ValidateResult validate_TARGET(TradeBillDTO bill) {
...
**WAIT_ORIGIN：** 退款落库时可置 `WAIT_ORIGIN`；原单清算成功后由 `ClearanceTaskServiceImpl.activateWaitingRefunds` 激活（**无独立 WaitOriginScanJob**）。
