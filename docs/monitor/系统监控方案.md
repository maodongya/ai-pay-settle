...
| 能力 | 现状 | 差距 |
|------|------|------|
| Prometheus 指标 | 基线 yml 已暴露 `/actuator/prometheus`（不限于 mq profile） | 部分设计指标仍未落地 |
| Outbox / 清算 Pending | `MqBacklogMonitorJob` 已实现 | 无告警去重；无最老 Pending 年龄指标 |
| MQ Topic Lag | 配置项 `lag-warn/lag-critical` 已存在 | Admin 采集有实现但失败常降级为 0 |
| DLQ 巡检 | `DlqInspectJob` + Admin collector | 失败时可能假健康；ACCESS_REFUND group 可能未纳入 |
| 告警落库 | `AlertService` → `alert_record` | 无企微/邮件/短信外发通道 |
| 日志告警 | 默认 Logback 控制台输出 | 无集中采集与规则匹配 |
| 表容量监控 | `DbCapacityMonitorJob` 已实现 | 告警仍仅落库+日志 |
