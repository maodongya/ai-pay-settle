...
public class AccountOperator {
    ...
    public MerchantSettleAccountEntity freeze(Long merchantId, String settleNo, BigDecimal amount) {
        return updateBalance(merchantId, null, settleNo, amount.negate(), AccountFlowOpType.FREEZE, false, amount); // 待结算减少，冻结增加
    }
    ...
    public MerchantSettleAccountEntity unfreeze(Long merchantId, String settleNo, BigDecimal amount) {
        return updateBalance(merchantId, null, settleNo, amount, AccountFlowOpType.UNFREEZE, false, amount.negate()); // 待结算增加，冻结减少
    }
    ...
    public MerchantSettleAccountEntity deductFrozen(Long merchantId, String settleNo, BigDecimal amount) {
        return updateFrozenOnly(merchantId, settleNo, amount.negate(), AccountFlowOpType.DEDUCT); // 仅扣减冻结余额
    }
    ...
    public MerchantSettleAccountEntity debit(Long merchantId, String billNo, BigDecimal amount) {
        if (accountFlowRepository.existsByBillNoAndOpType(billNo, AccountFlowOpType.REFUND_DEBIT.getCode())) { // 流水已存在
            return accountRepository.findByMerchantId(merchantId).orElseThrow(); // 幂等返回当前账户
        }
        return updateBalance(merchantId, billNo, null, amount.negate(), AccountFlowOpType.REFUND_DEBIT, true); // 扣减待结算余额
    }
